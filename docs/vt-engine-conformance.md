...
| **Cross-platform** | **Windows + Unix native (P7 landed)** | The `IPtyProcess` seam admits both `ConPtyProcess` (Windows/ConPTY) and `UnixPtyProcess` (`posix_openpt`+`posix_spawn`). Parser + grid stay 100% platform-neutral in the engine assembly, so P7 was *only* PTY plumbing — verified browserless on WSL. |
...
- **P7 (Unix PTY) ✅**: `UnixPtyProcess` is the cross-platform sibling of `ConPtyProcess`, behind a new `IPtyProcess` seam + `PtyProcess.Start` factory (Windows → ConPTY, Unix → `UnixPtyProcess`). It opens a real PTY with `posix_openpt`/`grantpt`/`unlockpt` and launches the child as a session leader on the slave via **`posix_spawn`** with `POSIX_SPAWN_SETSID` (not managed `forkpty` — forking a multi-threaded CLR and running managed code in the child is unsafe; `posix_spawn` does the fork+exec inside libc). The master fd is exposed as a duplex stream (read EIO → clean EOF on child exit). All three native consumers (`NativeTerminalRenderer`, `NativeRecordingSession`, `NativeTerminalPage`) route through the seam; the unspecified-shell default is now platform-aware (pwsh on Windows, bash on Unix), and `native-snap`/`native-play` run on Linux/macOS. Verified end-to-end on WSL: static + animated SVG, ANSI color, interactive Type/Key with prompt-`Wait`, Exec capture, and GIF rasterization. Parser + grid stay 100% platform-neutral in `VcrSharp.Terminal`.
  - **`Wait` after a non-interactive `Exec`:** a bare `Wait` defaults to the shell-prompt pattern (`>\s*$`), but a non-interactive `Exec` shell *exits* instead of re-prompting, so the prompt never comes. `NativeTerminalPage`'s wait loops now treat **child-exit as wait-satisfied** (brief tail-settle, then proceed) rather than blocking to the 15 s timeout and failing the recording — "the command is done." Inert for interactive tapes (shell stays alive → prompt-matching unchanged). Same on Windows and Unix.

### Native fallback rules (when `vcr demo.tape` must use the browser)
- **Statically detectable (pre-run):** *(future, once richer)* tapes using features the native engine doesn't claim. *(The native path now runs on Windows **and** Unix via the `IPtyProcess` seam, so the OS is no longer a fallback trigger; GIF/MP4/PNG rasterisation also works natively on both.)*
...
| **P7 — Unix PTY sibling ✅** | `UnixPtyProcess` (`posix_openpt` + `posix_spawn`/`POSIX_SPAWN_SETSID`) implementing the new `IPtyProcess` seam so the browserless path runs on Linux/macOS. Parser/grid were already platform-neutral (`VcrSharp.Terminal`); only PTY spawn was new. **Landed** — verified end-to-end on WSL (`native-snap`/`native-play`: static/animated SVG, color, interactive input, Exec, GIF). | All gates run cross-platform in CI | **M** |
