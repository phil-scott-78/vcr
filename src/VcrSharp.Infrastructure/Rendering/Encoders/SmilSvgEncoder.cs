...
/// <summary>
/// Default SVG encoder using SMIL animations for partial screen updates.
///
/// Unlike the legacy CSS-based SvgEncoder which uses a filmstrip approach (complete frames
/// side-by-side with translateX animation), this encoder:
...
///
/// This is the default for ".svg" output. Use ".legacy.svg" for the CSS filmstrip approach.
/// </summary>
public class SmilSvgEncoder(SessionOptions options, FrameStorage storage) : EncoderBase(options, storage)
{
    public override bool SupportsPath(string outputPath)
    {
        // Match .svg but not .legacy.svg (which uses CSS filmstrip approach)
        return Path.GetExtension(outputPath).Equals(".svg", StringComparison.OrdinalIgnoreCase)
            && !outputPath.EndsWith(".legacy.svg", StringComparison.OrdinalIgnoreCase);
    }
    ...
    public override async Task<string> RenderAsync(string outputPath, IProgress<string>? progress = null, CancellationToken cancellationToken = default)
    {
        ...
        // Generate SMIL SVG
        progress?.Report("Generating SMIL SVG with partial updates...");
        var renderer = new SmilSvgRenderer(Options);
